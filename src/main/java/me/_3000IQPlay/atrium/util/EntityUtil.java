...
class EntityUtil implements Util {
    ...
    List < EntityPlayer > getNearbyPlayers ( double d ) {
        ...
        list.removeIf ( entityPlayer -> Atrium.friendManager.isFriend ( entityPlayer.getName ( ) ) );
        return list;
    }
    ...
    boolean isntValid ( final Entity entity , final double range ) {
        return entity == null || isDead ( entity ) || entity.equals ( EntityUtil.mc.player ) || ( entity instanceof EntityPlayer && Atrium.friendManager.isFriend ( entity.getName ( ) ) ) || EntityUtil.mc.player.getDistanceSq ( entity ) > MathUtil.square ( range );
    }
    ...
    Color getColor ( final Entity entity , final int red , final int green , final int blue , final int alpha , final boolean colorFriends ) {
        ...
        if ( entity instanceof EntityPlayer ) {
            if ( colorFriends && Atrium.friendManager.isFriend ( (EntityPlayer) entity ) ) {
                color = new Color ( 0.33333334f , 1.0f , 1.0f , alpha / 255.0f );
            }
            final Killaura killaura = Atrium.moduleManager.getModuleByClass ( Killaura.class );
    ...
    Map < String, Integer > getTextRadarPlayers ( ) {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put ( healthSB + " " + ( Atrium.friendManager.isFriend ( player ) ? "\u00A7b" : "\u00A7r" ) + player.getName ( ) + " " + distanceSB + " " + "\u00A7f" + Atrium.totemPopManager.getTotemPopString ( player ) + Atrium.potionManager.getTextRadarPotion ( player ) , (int) EntityUtil.mc.player.getDistance ( player ) );
