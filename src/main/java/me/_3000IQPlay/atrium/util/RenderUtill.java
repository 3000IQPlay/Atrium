...
public class RenderUtill
    ...
    public static void drawWaypointImage(BlockPos pos, GLUProjection.Projection projection, Color color, String name, boolean rectangle, Color rectangleColor) {
        ...
        String text = name + ": " + Math.round(RenderUtill.mc.player.getDistance(pos.getX(), pos.getY(), pos.getZ())) + "M";
        float textWidth = Atrium.textManager.getStringWidth(text);
        Atrium.textManager.drawString(text, -(textWidth / 2.0f), -((float) Atrium.textManager.getFontHeight() / 2.0f) + (float) Atrium.textManager.getFontHeight() / 2.0f, color.getRGB(), false);
        GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
        GlStateManager.popMatrix();
    }
    ...
    public static void drawGradientPlane(BlockPos pos, EnumFacing face, Color startColor, Color endColor, boolean half, boolean top) {
        ...
        Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.player, mc.getRenderPartialTicks());
        AxisAlignedBB bb = iblockstate.getSelectedBoundingBox(RenderUtill.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z);
    ...
    public static void drawGradientPlane(BlockPos pos, EnumFacing face, Color startColor, Color endColor, double height) {
        ...
        IBlockState iblockstate = RenderUtill.mc.world.getBlockState(pos);
        Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.player, mc.getRenderPartialTicks());
        AxisAlignedBB bb = iblockstate.getSelectedBoundingBox(RenderUtill.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z).expand(0.0, height, 0.0);
        float red = (float) startColor.getRed() / 255.0f;
        float green = (float) startColor.getGreen() / 255.0f;
        float blue = (float) startColor.getBlue() / 255.0f;
        float alpha = (float) startColor.getAlpha() / 255.0f;
    ...
    public static void drawGradientBlockOutline(BlockPos pos, Color startColor, Color endColor, float linewidth, double height) {
        IBlockState iblockstate = RenderUtill.mc.world.getBlockState(pos);
        Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.player, mc.getRenderPartialTicks());
        RenderUtill.drawGradientBlockOutline(iblockstate.getSelectedBoundingBox(RenderUtill.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z).expand(0.0, height, 0.0), startColor, endColor, linewidth);
    }
    ...
    public static void drawProperGradientBlockOutline(BlockPos pos, Color startColor, Color midColor, Color endColor, float linewidth) {
        IBlockState iblockstate = RenderUtill.mc.world.getBlockState(pos);
        Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.player, mc.getRenderPartialTicks());
        RenderUtill.drawProperGradientBlockOutline(iblockstate.getSelectedBoundingBox(RenderUtill.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z), startColor, midColor, endColor, linewidth);
    }
    ...
    public static void drawGradientFilledBox(BlockPos pos, Color startColor, Color endColor) {
        IBlockState iblockstate = RenderUtill.mc.world.getBlockState(pos);
        Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.player, mc.getRenderPartialTicks());
        RenderUtill.drawGradientFilledBox(iblockstate.getSelectedBoundingBox(RenderUtill.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z), startColor, endColor);
    }
    ...
    public static void glScissor(float x, float y, float x1, float y1, ScaledResolution sr) {
        GL11.glScissor((int) (x * (float) sr.getScaleFactor()), (int) ((float) RenderUtill.mc.displayHeight - y1 * (float) sr.getScaleFactor()), (int) ((x1 - x) * (float) sr.getScaleFactor()), (int) ((y1 - y) * (float) sr.getScaleFactor()));
    }
    ...
    public static void drawBlockOutline(BlockPos pos, Color color, float linewidth, boolean air) {
        ...
        if ((air || iblockstate.getMaterial() != Material.AIR) && RenderUtill.mc.world.getWorldBorder().contains(pos)) {
            Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.player, mc.getRenderPartialTicks());
            RenderUtill.drawBlockOutline(iblockstate.getSelectedBoundingBox(RenderUtill.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z), color, linewidth);
        }
    }
    ...
    public static void drawBoxESP(BlockPos pos, Color color, float lineWidth, boolean outline, boolean box, int boxAlpha) {
        ...
        if (camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + RenderUtill.mc.getRenderManager().viewerPosX, bb.minY + RenderUtill.mc.getRenderManager().viewerPosY, bb.minZ + RenderUtill.mc.getRenderManager().viewerPosZ, bb.maxX + RenderUtill.mc.getRenderManager().viewerPosX, bb.maxY + RenderUtill.mc.getRenderManager().viewerPosY, bb.maxZ + RenderUtill.mc.getRenderManager().viewerPosZ))) {
            ...
            GL11.glHint(3154, 4354);
            GL11.glLineWidth(lineWidth);
            double dist = RenderUtill.mc.player.getDistance((float) pos.getX() + 0.5f, (float) pos.getY() + 0.5f, (float) pos.getZ() + 0.5f) * 0.75;
            if (box) {
    ...
    public static void drawText(BlockPos pos, String text) {
        ...
        GlStateManager.pushMatrix();
        RenderUtill.glBillboardDistanceScaled((float) pos.getX() + 0.5f, (float) pos.getY() + 0.5f, (float) pos.getZ() + 0.5f, RenderUtill.mc.player, 1.0f);
        GlStateManager.disableDepth();
        GlStateManager.translate(-((double) Atrium.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        Atrium.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
    ...
    public static void drawOutlinedBlockESP(BlockPos pos, Color color, float linewidth) {
        IBlockState iblockstate = RenderUtill.mc.world.getBlockState(pos);
        Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.player, mc.getRenderPartialTicks());
        RenderUtill.drawBoundingBox(iblockstate.getSelectedBoundingBox(RenderUtill.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z), linewidth, ColorUtil.toRGBA(color));
    }
    ...
    public static void glBillboard(float x, float y, float z) {
        ...
        GlStateManager.translate((double) x - RenderUtill.mc.getRenderManager().renderPosX, (double) y - RenderUtill.mc.getRenderManager().renderPosY, (double) z - RenderUtill.mc.getRenderManager().renderPosZ);
    ...
    public static void drawBar(GLUProjection.Projection projection, float width, float height, float totalWidth, Color startColor, Color outlineColor) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            RenderUtill.drawOutlineRect(-(totalWidth / 2.0f), -(height / 2.0f), totalWidth, height, outlineColor.getRGB());
            RenderUtill.drawRect(-(totalWidth / 2.0f), -(height / 2.0f), width, height, startColor.getRGB());
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
        }
    }
    ...
    public static void drawProjectedText(GLUProjection.Projection projection, float addX, float addY, String text, Color color, boolean shadow) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            Atrium.textManager.drawString(text, -((float) Atrium.textManager.getStringWidth(text) / 2.0f) + addX, addY, color.getRGB(), shadow);
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
        }
    }
    ...
    public static void drawChungusESP(GLUProjection.Projection projection, float width, float height, ResourceLocation location) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            ...
            GlStateManager.disableBlend();
            mc.getTextureManager().bindTexture(location);
            RenderUtill.drawCompleteImage(0.0f, 0.0f, width, height);
