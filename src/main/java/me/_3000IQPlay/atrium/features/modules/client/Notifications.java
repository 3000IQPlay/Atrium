...
import me._3000IQPlay.atrium.manager.FileManager;
...
public class Notifications
        extends Module {
    private static final String fileName = "atrium/util/ModuleMessage_List.txt";
    ...
    public Setting<Integer> delay = this.register(new Setting<Object>("Delay", 500, 0, 5000, v -> this.totemPops.getValue(), "Delays messages."));
    ...
    public Setting<Boolean> watermark = this.register(new Setting<Object>("Watermark", Boolean.valueOf(false), v -> this.moduleMessage.getValue()));
    ...
    public TextComponentString getNotifierOn(Module module) {
        if (ModuleTools.getInstance().isEnabled()) {
            switch (ModuleTools.getInstance().notifier.getValue()) {
                ...
                case ATRIUM: {
                    TextComponentString text = new TextComponentString((Atrium.commandManager.getClientMessage()) + ChatFormatting.BOLD + module.getDisplayName() + ChatFormatting.RESET + ChatFormatting.WHITE + " was" + ChatFormatting.GREEN + " enabled.");
                    return text;

                }
            }
        }
        TextComponentString text = new TextComponentString(Atrium.commandManager.getClientMessage() + ChatFormatting.GREEN + module.getDisplayName() + " toggled on.");
        return text;
    }
    ...
    public TextComponentString getNotifierOff(Module module) {
        if (ModuleTools.getInstance().isEnabled()) {
            switch (ModuleTools.getInstance().notifier.getValue()) {
                ...
                case ATRIUM: {
                    TextComponentString text = new TextComponentString((Atrium.commandManager.getClientMessage()) + ChatFormatting.BOLD + module.getDisplayName() + ChatFormatting.RESET + ChatFormatting.WHITE + " was" + ChatFormatting.RED + " disabled.");
                    return text;

                }
            }
        }
        TextComponentString text = new TextComponentString(Atrium.commandManager.getClientMessage() + ChatFormatting.RED + module.getDisplayName() + " toggled off.");
        return text;
    }
