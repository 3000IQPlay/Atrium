...
public class HoleFiller
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            Atrium.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            Atrium.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = Atrium.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Atrium.holeManager.getMidSafety());
            }
        }
        object = Atrium.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(Atrium.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && Atrium.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
