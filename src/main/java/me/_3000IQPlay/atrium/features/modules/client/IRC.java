...
import me._3000IQPlay.atrium.manager.WaypointManager;
...
public class IRC
    ...
    public static List<String> atriumUsers;
    ...
    public Setting<Integer> blue = this.register(new Setting<Object>("Blue", Integer.valueOf(255), Integer.valueOf(0), Integer.valueOf(255), v -> this.waypoints.getValue()));
    ...
    public IRC() {
        super("AtriumChat", "Atrium chat server", Module.Category.CLIENT, true, false, true);
        INSTANCE = this;
    }
    ...
    public static void updateInventories() {
        for (final String player : IRC.atriumUsers) {
            try {
    ...
    public void onUpdate() {
        ...
        if (!IRC.mc.isSingleplayer() && !(IRC.mc.currentScreen instanceof AtriumGui) && IRC.handler != null && !IRC.handler.socket.isClosed() && this.status) {
            if (this.down) {
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        if (this.waypoints.getValue()) {
            for (final WaypointManager.Waypoint waypoint : Atrium.waypointManager.waypoints.values()) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        if (this.inventories.getValue()) {
            ...
            this.textRadarY = 0;
            for (final String player : IRC.atriumUsers) {
                if (Atrium.inventoryManager.inventories.get(player) != null) {
                    continue;
                }
                final List<ItemStack> stacks = Atrium.inventoryManager.inventories.get(player);
                            ...
                            if (input.equalsIgnoreCase("list")) {
                                ...
                                for (final String friend : split2) {
                                    if (!friend.equals(Util.mc.player.getName())) {
                                        Atrium.friendManager.addFriend(friend);
                                ...
                                Atrium.waypointManager.waypoints.put(name, new WaypointManager.Waypoint(name, server, Integer.parseInt(dimension), Integer.parseInt(inputs[2]), Integer.parseInt(inputs[3]), Integer.parseInt(inputs[4]), color));
                                ...
                                final String name = this.inputStream.readUTF();
                                Atrium.waypointManager.waypoints.remove(name);
                                ...
                                for (final String player : IRC.atriumUsers) {
                                    if (player.equalsIgnoreCase(name)) {
                                        Atrium.inventoryManager.inventories.put(player, IRC.deserializeInventory(inventory));
                                    }
                                }
                                ...
                                for (final String name2 : players) {
                                    ...
                                    if (!IRC.atriumUsers.contains(name2)) {
                                        IRC.atriumUsers.add(name2);
                                    }
                                }
                            }
