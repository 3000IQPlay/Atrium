...
import me._3000IQPlay.atrium.util.TextUtil;
...
public class Managers
    ...
    public Setting<String> command = this.register(new Setting<String>("Command", "Atrium"));
    public Setting<Boolean> rainbowPrefix = this.register(new Setting<Boolean>("RainbowPrefix", false));
    public Setting<TextUtil.Color> bracketColor = this.register(new Setting<TextUtil.Color>("BColor", TextUtil.Color.BLACK));
    public Setting<TextUtil.Color> commandColor = this.register(new Setting<TextUtil.Color>("CColor", TextUtil.Color.AQUA));
    ...
    @Override
    public void onLoad() {
        Atrium.commandManager.setClientMessage(this.getCommandMessage());
    }
        ...
        if (event.getStage() == 2) {
            ...
            if (event.getSetting() != null && this.equals(event.getSetting().getFeature())) {
                if (event.getSetting().equals(this.holeThread)) {
                    Atrium.holeManager.settingChanged();
                }
                Atrium.commandManager.setClientMessage(this.getCommandMessage());
            }
        }
