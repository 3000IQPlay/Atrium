...
import me._3000IQPlay.atrium.features.modules.misc.ToolTips;
...
public class HUD
    ...
    private static final ResourceLocation codHitmarker = new ResourceLocation("atriumsounds", "cod_hitmarker");
    private static final ResourceLocation csgoHitmarker = new ResourceLocation("atriumsounds", "csgo_hitmarker");
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "Atrium", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public Setting<Integer> hudRed = this.register(new Setting<Object>("Red", Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(255), v -> this.rainbow.getValue() == false));
    public Setting<Integer> hudGreen = this.register(new Setting<Object>("Green", Integer.valueOf(255), Integer.valueOf(0), Integer.valueOf(255), v -> this.rainbow.getValue() == false));
    ...
    public void onUpdate() {
        for (final Module module : Atrium.moduleManager.sortedModules) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Atrium.moduleManager.alphabeticallySortedModules.size() : Atrium.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Atrium.moduleManager.alphabeticallySortedModules.get(k) : Atrium.moduleManager.sortedModules.get(k);
                    ...
                    final Color moduleColor = Atrium.moduleManager.moduleColorMap.get(module);
                ...
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Atrium.moduleManager.alphabeticallySortedModules.size() : Atrium.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Atrium.moduleManager.alphabeticallySortedModules.get(Atrium.moduleManager.alphabeticallySortedModules.size() - 1 - k) : Atrium.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + ChatFormatting.GRAY + ((module.getDisplayInfo() != null) ? (" [" + ChatFormatting.WHITE + module.getDisplayInfo() + ChatFormatting.GRAY + "]") : "");
                    final Color moduleColor = Atrium.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + ChatFormatting.WHITE + Atrium.serverManager.getServerBrand();
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : Atrium.potionManager.getOwnPotions()) {
                    final String text3 = this.altPotionsColors.getValue() ? Atrium.potionManager.getPotionString(effect) : Atrium.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + ChatFormatting.WHITE + Atrium.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + ChatFormatting.WHITE + Atrium.serverManager.getTPS();
            ...
            final String fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.debugFPS;
            final String text = grayString + "Ping " + ChatFormatting.WHITE + Atrium.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
        ...
        if (this.renderingUp.getValue()) {
            Atrium.notificationManager.handleNotifications(height - (k + 16));
        } else {
            Atrium.notificationManager.handleNotifications(height - (j + 16));
        }
        ...
        final String text4 = (this.direction.getValue() ? (Atrium.rotationManager.getDirection4D(false) + " ") : "") + (this.coords.getValue() ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to Atrium " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
    ...
    public void renderLag() {
        final int width = this.renderer.scaledWidth;
        if (Atrium.serverManager.isServerNotResponding()) {
            final String text = ((this.lag.getValue() == LagNotify.GRAY) ? ChatFormatting.GRAY : ChatFormatting.RED) + "Server not responding: " + MathUtil.round(Atrium.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
