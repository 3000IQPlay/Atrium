...
import me._3000IQPlay.atrium.features.modules.misc.NoSoundLag;
...
public class AutoCrystal
    ...
    public Setting<Integer> wasteAmount = this.register(new Setting<Object>("WasteAmount", 5, 1, 5, v -> this.setting.getValue() == Settings.PLACE && this.place.getValue()));
    ...
    private final Setting<Integer> bRed = this.register(new Setting<Object>("BoxRed", 0, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));
    private final Setting<Integer> bGreen = this.register(new Setting<Object>("BoxGreen", 255, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));
    private final Setting<Integer> bBlue = this.register(new Setting<Object>("BoxBlue", 255, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.box.getValue()));
    ...
    private final Setting<Integer> oRed = this.register(new Setting<Object>("OutlineRed", 0, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));
    private final Setting<Integer> oGreen = this.register(new Setting<Object>("OutlineGreen", 255, 0, 255, v -> this.setting.getValue() == Settings.RENDER && this.render.getValue() && this.outline.getValue()));
    ...
    public Setting<Logic> logic = this.register(new Setting<Object>("Logic", Logic.PLACEBREAK, v -> this.setting.getValue() == Settings.DEV));
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue() <= MathUtil.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue()))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue() + this.placeRange.getValue()) || !Atrium.friendManager.isFriend(friend) || !((double) DamageUtil.calculateDamage(pos, friend) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
    ...
    private boolean isRightThread() {
        return mc.isCallingFromMinecraftThread() || !Atrium.eventManager.ticksOngoing() && !this.threadOngoing.get();
    }
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if ((this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC) && this.renderPos != null && (this.box.getValue() || this.outline.getValue())) {
            ...
            if (this.renderMode.getValue() == RenderMode.STATIC) // normal mode
                RenderUtil.drawSexyBoxAtriumIsRetardedFuckYouESP(
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof AtriumGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue() && this.offhandSwitch.getValue() && this.offHand) {
                Offhand module = Atrium.moduleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !Atrium.friendManager.isFriend(player.getName()) || !((double) DamageUtil.calculateDamage(entity, player) > (double) EntityUtil.getHealth(player) + 0.5))
                        continue;
        ...
        switch (this.rotate.getValue()) {
            ...
            case ALL: {
                ...
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Atrium.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
                this.yaw = angle[0];
                this.pitch = angle[1];
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    while (Atrium.eventManager.ticksOngoing()) {
                    }
                    ...
                    Atrium.safetyManager.doSafetyCheck();
