...
import me._3000IQPlay.atrium.event.events.*;
...
public class EventManager
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!EventManager.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals(EventManager.mc.player)) {
            Atrium.potionManager.update();
            Atrium.totemPopManager.onUpdate();
            Atrium.inventoryManager.update();
            Atrium.holeManager.update();
            Atrium.safetyManager.onUpdate();
            Atrium.moduleManager.onUpdate();
            Atrium.timerManager.update();
            if (this.timer.passedMs(Managers.getInstance().moduleListUpdates.getValue().intValue())) {
                Atrium.moduleManager.sortModules(true);
                Atrium.moduleManager.alphabeticallySortModules();
                this.timer.reset();
            }
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        Atrium.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        Atrium.moduleManager.onLogout();
        Atrium.totemPopManager.onLogout();
        Atrium.potionManager.onLogout();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            Atrium.baritoneManager.onUpdateWalkingPlayer();
            Atrium.speedManager.updateValues();
            Atrium.rotationManager.updateRotations();
            Atrium.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            Atrium.rotationManager.restoreRotations();
            Atrium.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        Atrium.serverManager.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(EventManager.mc.world) instanceof EntityPlayer) {
                ...
                Atrium.totemPopManager.onTotemPop(player);
                Atrium.potionManager.onTotemPop(player);
            }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        EventManager.mc.profiler.startSection("atrium");
        ...
        Atrium.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            Atrium.textManager.updateResolution();
        }
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            Atrium.moduleManager.onRender2D(render2DEvent);
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    Atrium.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
