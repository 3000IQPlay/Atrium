...
import me._3000IQPlay.atrium.features.modules.render.XRay;
...
public class ConfigManager
    ...
    public String config = "atrium/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                Atrium.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Atrium.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("atrium").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("atrium/" + name + "/")) ? "atrium/" + name + "/" : "atrium/config/";
    ...
    public void saveConfig(String name) {
        ...
        this.config = "atrium/" + name + "/";
        File path = new File(this.config);
        if (!path.exists()) {
            path.mkdir();
        }
        Atrium.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("atrium/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("atrium", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(Atrium.moduleManager.modules);
        this.features.add(Atrium.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Atrium.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Atrium.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
